/**
 * 示例 4: 用户资料（异步加载，useStore + 手动 loading/error）
 */
...
export function UserProfile({userId}: {userId: string}) {
  const [snap, store] = useStore(() => ({
    ...
    posts: [] as Post[],
    loading: {fetchUser: false, fetchPosts: false},
    error: {fetchUser: null as unknown, fetchPosts: null as unknown},
  ...
  const fetchUser = async (id: string) => {
    store.loading.fetchUser = true
    store.error.fetchUser = null
    try {
      ...
      store.error.fetchUser = e
      throw e
    } finally {
      store.loading.fetchUser = false
    }
  }

  const fetchPosts = async (uid: string) => {
    store.loading.fetchPosts = true
    store.error.fetchPosts = null
    try {
      ...
      store.error.fetchPosts = e
      throw e
    } finally {
      store.loading.fetchPosts = false
    }
  }
